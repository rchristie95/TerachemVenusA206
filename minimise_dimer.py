...
This variant renders the video from minimization-progress frames instead of NVT frames.
"""
...
def overlay_energy_on_frames(
    ...
    if not frame_paths or not energies:
        return
    ...
    n_frames = min(len(frame_paths), len(energies))
    if len(frame_paths) != len(energies):
        print(
            f"[!] Energy overlay length mismatch: {len(frame_paths)} frames, {len(energies)} energies. "
            f"Overlaying first {n_frames}."
        )
    ...
    for idx in range(n_frames):
        ...
        with Image.open(frame_path) as img:
            ...
            x = max(margin_px, base.size[0] - text_w - margin_px)
            y = margin_px
            draw.rectangle(
...
def render_video(
    ...
    pymol_launch: str,
    frame_energies: Optional[List[float]] = None,
) -> int:
    ...
    if frame_energies:
        print("[*] Overlaying energy labels on frames...")
        overlay_energy_on_frames(frame_paths, frame_energies)
...
def parse_args(argv=None):
    ...
    parser.add_argument("--workdir", type=Path, default=Path("tc_dimer_minimise"), help="Output workdir")
    parser.add_argument(
        "--overwrite-workdir",
        action="store_true",
        default=True,
    ...
    parser.add_argument("--temperature-k", type=float, default=300.0, help="NVT temperature (K)")
    ...
    parser.add_argument("--nvt-steps", type=int, default=0, help="NVT MD steps")
    parser.add_argument("--minimize-iters", type=int, default=100000, help="Max minimization iterations")
    ...
    parser.add_argument(
        ...
        default="dimer_minimise_trajectory.pdb",
        help="Minimization trajectory filename saved in workdir",
    )
    parser.add_argument(
        "--openmm-trajectory-interval",
        type=int,
        default=10,
        help="Minimization report interval in L-BFGS iterations",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Render a video from the generated dimer minimization trajectory (default: enabled)",
    )
    parser.add_argument(
        "--no-video",
        dest="make_video",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output video path (default: <workdir>/dimer_minimise.mp4)",
    )
    parser.add_argument(
        "--video-args",
        default="",
    ...
    parser.add_argument("--frames-dir", type=Path, default=Path("minimization_frames_minimise"))
...
def run_openmm_nvt(args) -> tuple[Path | None, Path, Path, List[CycleTrajectory], List[float]]:
    ...
    minimize_report_interval = int(args.openmm_trajectory_interval) if args.openmm_trajectory_interval else 0
    openmm_traj_path = None
    min_traj_handle = None
    min_traj_models_written = 0
    if minimize_report_interval > 0:
        openmm_traj_path = workdir / args.openmm_trajectory_file
        min_traj_handle = openmm_traj_path.open("w", encoding="utf-8")
        PDBFile.writeHeader(modeller.topology, file=min_traj_handle)
    ...
    if hasattr(openmm, "MinimizationReporter"):
        class _MinimizationTrajectoryReporter(openmm.MinimizationReporter):
            def __init__(
                self,
                label: str,
                interval: int,
                trajectory_handle=None,
                trajectory_topology=None,
                model_index_start: int = 1,
                cycle_dir: Optional[Path] = None,
                cycle_prefix: str = "min",
            ):
                super().__init__()
                ...
                self._cycle = 1
                self._trajectory_handle = trajectory_handle
                self._trajectory_topology = trajectory_topology
                self._model_index = int(model_index_start)
                self._combined_energies: List[float] = []
                self._cycle_dir = Path(cycle_dir).resolve() if cycle_dir is not None else None
                self._cycle_prefix = str(cycle_prefix)
                self._cycle_records: List[CycleTrajectory] = []
                self._cycle_handle = None
                self._cycle_path: Optional[Path] = None
                self._cycle_energies: List[float] = []
                self._cycle_model_index = 1
                self._current_cycle = 1
                if self._cycle_dir is not None and self._trajectory_topology is not None:
                    self._cycle_dir.mkdir(parents=True, exist_ok=True)
                    self._open_cycle_file(self._cycle)
            ...
            @property
            def combined_energies(self) -> List[float]:
                return list(self._combined_energies)

            @property
            def cycle_records(self) -> List[CycleTrajectory]:
                return list(self._cycle_records)

            @staticmethod
            def _positions_from_flat_nm(flat_coords):
                values = list(flat_coords)
                positions = []
                ...
                # OpenMM minimizer reports coordinates in nanometers; attach units so PDB output is in Angstroms.
                return positions * unit.nanometer

            def _open_cycle_file(self, cycle_index: int) -> None:
                if self._cycle_dir is None or self._trajectory_topology is None:
                    return
                path = self._cycle_dir / f"{self._cycle_prefix}_cycle{cycle_index:02d}.pdb"
                handle = path.open("w", encoding="utf-8")
                PDBFile.writeHeader(self._trajectory_topology, file=handle)
                self._cycle_handle = handle
                self._cycle_path = path
                self._cycle_energies = []
                self._cycle_model_index = 1
                self._current_cycle = int(cycle_index)

            def _close_cycle_file(self) -> None:
                if self._cycle_handle is None or self._trajectory_topology is None or self._cycle_path is None:
                    return
                try:
                    PDBFile.writeFooter(self._trajectory_topology, file=self._cycle_handle)
                finally:
                    self._cycle_handle.close()
                if self._cycle_energies:
                    self._cycle_records.append(
                        CycleTrajectory(
                            label=self._label,
                            tag=self._cycle_prefix,
                            cycle=int(self._current_cycle),
                            traj_path=self._cycle_path,
                            energies=list(self._cycle_energies),
                        )
                    )
                else:
                    try:
                        self._cycle_path.unlink()
                    except Exception:
                        pass
                self._cycle_handle = None
            ...
            def report(self, iteration, x, grad, stats=None):
                ...
                if self._frame == 1 or (self._frame % self._interval == 0):
                    ...
                    positions = None
                    if self._trajectory_topology is not None and (self._trajectory_handle is not None or self._cycle_handle is not None):
                        positions = self._positions_from_flat_nm(x)
                    if self._trajectory_handle is not None and self._trajectory_topology is not None:
                        PDBFile.writeModel(
                            self._trajectory_topology,
                            positions,
                            file=self._trajectory_handle,
                            modelIndex=self._model_index,
                            keepIds=True,
                        )
                        self._trajectory_handle.flush()
                        self._model_index += 1
                    if self._cycle_handle is not None and self._trajectory_topology is not None:
                        PDBFile.writeModel(
                            self._trajectory_topology,
                            positions,
                            file=self._cycle_handle,
                            modelIndex=self._cycle_model_index,
                            keepIds=True,
                        )
                        self._cycle_handle.flush()
                        self._cycle_model_index += 1
                        self._cycle_energies.append(energy)
                    self._combined_energies.append(energy)
                    print(
                        f"      [OpenMM min {self._label}] frame={self._frame} "
                        f"step={iteration} cycle={self._cycle} energy={energy:.6f} kJ/mol",
                        flush=True,
                    )
                return False
    else:
        _MinimizationTrajectoryReporter = None

    def run_minimization(label: str, model_index_start: int) -> tuple[int, int, List[CycleTrajectory], List[float]]:
        if can_stream_minimization:
            if _MinimizationTrajectoryReporter is None:
                simulation.minimizeEnergy(maxIterations=args.minimize_iters)
                return 0, model_index_start, [], []
            tag = "".join(ch for ch in str(label) if ch.isdigit())
            cycle_prefix = f"min{tag}" if tag else str(label).replace("#", "").strip() or "min"
            reporter = _MinimizationTrajectoryReporter(
                label=label,
                interval=minimize_report_interval,
                trajectory_handle=min_traj_handle,
                trajectory_topology=modeller.topology,
                model_index_start=model_index_start,
                cycle_dir=workdir / "minimization_cycles",
            ...
            try:
                ...
                reporter.finalize()
            return reporter.frame_count, reporter.model_index, reporter.cycle_records, reporter.combined_energies
    ...
    if minimize_report_interval > 0 and not can_stream_minimization:
        print("    - Warning: minimization streaming unsupported by this OpenMM build; no frames will be written")

    model_index = 1
    min1_frames = 0
    min2_frames = 0
    cycle_trajs: List[CycleTrajectory] = []
    combined_energies: List[float] = []
    e_before_min1 = potential_kj_per_mol()
    e_after_min1 = e_before_min1
    e_before_min2 = e_before_min1
    e_after_min2 = e_before_min1
    try:
        min1_frames, model_index, min1_cycles, min1_energies = run_minimization("#1", model_index)
        cycle_trajs.extend(min1_cycles)
        combined_energies.extend(min1_energies)
        ...
        min2_frames, model_index, min2_cycles, min2_energies = run_minimization("#2", model_index)
        cycle_trajs.extend(min2_cycles)
        combined_energies.extend(min2_energies)
        e_after_min2 = potential_kj_per_mol()
        min_traj_models_written = model_index - 1
    finally:
        if min_traj_handle is not None:
            try:
                PDBFile.writeFooter(modeller.topology, file=min_traj_handle)
            finally:
                min_traj_handle.close()
                min_traj_handle = None
    ...
    if can_stream_minimization:
        print(f"    - Minimization #1 streamed frames: {min1_frames} (interval={minimize_report_interval} iteration(s))")
        print(f"    - Minimization #2 streamed frames: {min2_frames} (interval={minimize_report_interval} iteration(s))")
    print(f"    - Saved: {relaxed_pdb}")
    if openmm_traj_path is not None:
        if min_traj_models_written > 0:
            print(
                f"    - OpenMM minimization trajectory: {openmm_traj_path} "
                f"({min_traj_models_written} models, interval={minimize_report_interval} iteration(s))"
            )
        else:
            print(f"    - OpenMM minimization trajectory requested but no frames were written: {openmm_traj_path}")

    return openmm_traj_path, workdir, relaxed_pdb, cycle_trajs, combined_energies
...
def main(argv=None) -> int:
    ...
    try:
        traj_path, workdir, _, cycle_trajs, combined_energies = run_openmm_nvt(args)
    except Exception as exc:
        print(f"[ERROR] {exc}")
        return 1
    ...
    if args.make_video:
        ...
        if video_output is None:
            video_output = workdir / "dimer_minimise.mp4"
        video_output = resolve_path(video_output)

        if cycle_trajs:
            base_ext = video_output.suffix or ".mp4"
            base_stem = video_output.stem if video_output.suffix else video_output.name
            base_dir = video_output.parent
            base_dir.mkdir(parents=True, exist_ok=True)

            print("[*] Rendering per-cycle minimization videos...")
            for cycle in cycle_trajs:
                out_name = f"{base_stem}_{cycle.tag}_cycle{cycle.cycle:02d}{base_ext}"
                out_path = base_dir / out_name
                frames_dir = resolve_path(args.frames_dir) / f"{cycle.tag}_cycle{cycle.cycle:02d}"
                code = render_video(
                    traj_path=cycle.traj_path,
            ...
            if traj_path is None:
                print("[ERROR] Minimization trajectory was not written; cannot render video.")
                return 1
            if not traj_path.exists() or traj_path.stat().st_size == 0:
                print(f"[ERROR] Minimization trajectory missing/empty: {traj_path}")
                return 1

            frames_dir = resolve_path(args.frames_dir)

            print("[*] Rendering dimer minimization video...")
            code = render_video(
                traj_path=traj_path,
                ...
                allow_gif_fallback=bool(args.allow_gif_fallback),
                pymol_launch=str(args.pymol_launch),
                frame_energies=combined_energies if combined_energies else None,
            )
            if code != 0:
                return code
