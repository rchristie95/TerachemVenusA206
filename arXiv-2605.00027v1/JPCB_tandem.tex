...
\begin{abstract}
We quantify the excitonic coupling in the homodimer of dimeric Venus fluorescent protein using a quantum-classical hybrid workflow. Because the anionic chromophore carries appreciable double-excitation character, we obtain its site energy from domain-based local-pair-natural-orbital similarity-transformed equation-of-motion coupled-cluster theory (DLPNO-STEOM-CCSD), which places the in-protein bright $\pi\pi^{*}$ state at $527~\mathrm{nm}$ ($2.35~\mathrm{eV}$), in good agreement with the experimental absorption maximum near $515~\mathrm{nm}$ and validated against an explicit triples-corrected EOM-CCSD(fT) benchmark; time-dependent density functional theory (TDDFT), a single-excitation theory, blueshifts this state for the present chromophore and cannot represent its double-excitation character. Employing a transition-density coupling formalism on the STEOM transition density, we calculate $J \approx 100~\mathrm{cm^{-1}}$ at the energy-minimised geometry. Simulating the \emph{actual} VenusA206 tandem-dimer construct---two barrels covalently joined by the experimental inter-domain linker---the covalent tether holds the A206 interface docked throughout, and the coupling remains stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$, roughly $4.4$ times stronger than the far-field point-dipole estimate of $23~\mathrm{cm^{-1}}$ and reflecting near-field multipolar effects at the ${\sim}25~\AA\ $ chromophore separation. Furthermore, we argue that a separation of timescales resolves the apparent theoretical tension between intermediate experimental excitonic couplings and the highly decoherent biological environment. While it has been hypothesised that the fluorescent protein $\beta$-barrel scaffold sustains coupling by attenuating thermal fluctuations, we emphasise that the separation of timescales applies irrespective of the exact degree of environmental noise suppression. Collective photoexcitation imprints the Davydov splitting under optical-limit dielectric screening upon absorption, preceding bulk solvent relaxation and sub-picosecond environmental dephasing. To characterise the subsequent post-absorption evolution, we employ stochastic simulations for quantum parts to model the transition from a delocalised exciton superposition to incoherent hopping between localised chromophore states.
\end{abstract}
...
Homodimers of the yellow fluorescent protein variant dimeric Venus challenge this assumption. Operating at room temperature, these dimers exhibit distinct Davydov splitting in circular dichroism (CD) spectra, and photon antibunching statistics confirm that the interacting chromophores act as a collective quantum entity upon photon absorption \cite{kim2019venusa206, freed2025effect}. Independent evidence comes from the two-photon polarization ratio, which resolves the strongly overlapping excitonic J- and H-transitions of the Venus dimer and constrains the mutual geometrical arrangement of the two chromophores \cite{cusick2026polarization}. These signatures indicate an interaction strength firmly within the intermediate excitonic regime. {Couplings of this magnitude, and substantially larger, are well documented in photosynthetic pigment--protein complexes~\cite{scholes2003, van2000photosynthetic, ishizaki2009unified}; the open question here is therefore not the magnitude of $J$ \emph{per se} but the persistence of its coherent spectroscopic signatures at room temperature, in an environment otherwise expected to induce strong decoherence.}
...
\begin{table}[t]
\centering
\caption{Vertical excitation energy of the bright $\pi\pi^{*}$ state of the anionic Venus chromophore. TDDFT yields a single visible bright root (all higher roots dark, $f<0.05$) but blueshifts it substantially relative to the correlated DLPNO-STEOM-CCSD result and to experiment; STEOM-CCSD, adopted here as the method of record, agrees with the measured absorption. In-protein values include the point-charge (MM) embedding described in the Methods.}
\label{tab:roots}
{\small\setlength{\tabcolsep}{3pt}
\begin{tabular}{lccc}
\toprule
Method & $\Delta E$ (eV) & $\lambda$ (nm) & $f$ \\
\midrule
TDDFT, $\omega$B97X-D3      & 3.13 & 396 & 1.35 \\
DLPNO-STEOM-CCSD            & 2.35 & 527 & 0.85 \\
Experiment (absorption)    & 2.41 & $\sim$515 & --- \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[t]
\centering
\caption{Method calibration on the gas-phase (neutral) chromophore, def2-SVP. Bare EOM-CCSD overestimates the excitation energy; the non-iterative triples correction of EOM-CCSD(fT) shifts it by $-0.43$~eV onto the DLPNO-STEOM-CCSD value, and ADC(2) assigns ${\sim}12\%$ doubles (2p2h) character to the transition. STEOM-CCSD thus reproduces the triples-corrected benchmark, confirming that it captures the double-excitation physics that single-excitation TDDFT cannot represent.}
\label{tab:ladder}
\begin{tabular}{lc}
\toprule
Method & $\Delta E$ (eV) \\
\midrule
EOM-CCSD (bare)          & 3.72 \\
EOM-CCSD(fT) (+triples)  & 3.29 \\
DLPNO-STEOM-CCSD         & 3.34 \\
\bottomrule
\end{tabular}
\end{table}
...
The chromophore possesses a single bright excitation in the visible: TDDFT (TeraChem) returns one dominant $\pi\pi^{*}$ root carrying essentially all of the oscillator strength ($f=1.35$), with every higher root dark ($f<0.05$). However, as a single-excitation theory TDDFT cannot represent the ${\sim}12\%$ double-excitation character of this transition (Table~\ref{tab:ladder}) and, for the anionic chromophore, blueshifts the bright state to $396~\mathrm{nm}$ ($3.13~\mathrm{eV}$)---roughly $0.7~\mathrm{eV}$ to the blue of experiment. We therefore adopt the correlated wavefunction result as the site energy: DLPNO-STEOM-CCSD (ORCA \cite{neese2020orca,neese2022orca,nooijen1997steom,dutta2018ipeom,dutta2019eaeom,dutta2016dlpno}) places the in-protein bright state at $2.35~\mathrm{eV}$ ($527~\mathrm{nm}$, $f=0.85$), in good agreement with the experimental absorption maximum near $515~\mathrm{nm}$ (Table~\ref{tab:roots}). This assignment is validated on the tractable gas-phase (neutral) chromophore, where a non-iterative triples correction shifts bare EOM-CCSD by $-0.43~\mathrm{eV}$ onto the STEOM value (Table~\ref{tab:ladder}), confirming that STEOM-CCSD captures the double-excitation physics that bare EOM-CCSD, ADC(2) and TDDFT miss \cite{manohar2008ft,epifanovsky2021qchem}. To physically characterise the transition, the STEOM electron transition density ($\rho^{g\to e}$) and the ground-to-excited state electron density difference ($\rho^{e} - \rho^{g}$) were mapped onto the dimer frame (\textbf{Figure~\ref{fig:density_combined}} and \textbf{Figure~\ref{fig:electron_combined}}, respectively).
...
Then, the STEOM transition densities were mapped to the dimer frame (chromophore centroid separation $25.5~\mathrm{\AA}$; inter-dipole angle $104^{\circ}$). The optical-limit coupling at the energy-minimised geometry ($t=0$) is summarised as

{\sloppy\setlength{\leftmargini}{1em}
\begin{itemize}
    \item \textbf{Transition Density Coupling:} $J_{\text{TDC}}(0) = 100~\mathrm{cm^{-1}}$ ($12.4~\mathrm{meV}$),
    \item \textbf{Point-Dipole Approximation:} $J_{\text{PDA}}(0) = 23~\mathrm{cm^{-1}}$ ($2.85~\mathrm{meV}$).
\end{itemize}}

\noindent For comparison, evaluating the same TDC pipeline on the TDDFT transition density of the identical transition gives $J_{\text{TDC}} = 74~\mathrm{cm^{-1}}$ over $J_{\text{PDA}} = 13~\mathrm{cm^{-1}}$; the near-field enhancement over the PDA is thus a robust, method-consistent feature, with the more strongly peaked STEOM density coupling ${\sim}1.4$ times more strongly than TDDFT at fixed geometry.
...
The disparity between the TDC calculation and the far-field point-dipole estimate provides a first-principles rationale for the appreciable strength of this excitonic coupling. Our structural model evaluates the coupling at a chromophore centroid separation of $25.5~\mathrm{\AA}$. At this distance, the PDA severely underestimates the interaction strength by a factor of $\sim 4.4$ (and by $\sim 5.6$ for the TDDFT density). While the classical point-dipole model predicts a rapid inverse-cube distance decay, the spatially extended transition density in \textbf{Figure~\ref{fig:density_combined}} enables specific phase lobes to interact at effective distances significantly shorter than the centroid separation, while remaining beyond the range of significant Dexter exchange \cite{skourtis2016dexter}.

To identify the origin of this enhancement, we decomposed the coupling into a primitive Cartesian multipole series about each chromophore centroid (companion analysis \texttt{multipole\_analysis.py}, evaluated on the transition density of the bright $\pi\pi^{*}$ state), evaluating the transition dipole, quadrupole and octupole moments and the corresponding inter-site interaction terms (dipole--dipole, dipole--quadrupole, quadrupole--dipole, dipole--octupole, quadrupole--quadrupole, octupole--dipole). The dipole--dipole term reproduces $J_{\text{PDA}}$ exactly by construction, but the truncated series converges only slowly at this geometry: summing all terms through octupole recovers just $\sim 28\%$ of the full $J_{\text{TDC}}$ (dipole alone $\sim 18\%$, through quadrupole $\sim 26\%$). The majority of the near-field enhancement therefore does not reside in a few leading multipoles but in the short-range overlap of the spatially extended transition densities---whose closest inter-chromophore approach ($\sim 15~\mathrm{\AA}$) is far smaller than the centroid separation---which only the full transition-density integral captures. This sharpens the broader lesson for the PDA: when transition densities are delocalised and their nearest approach is comparable to the molecular size, even a multipole-corrected dipole model is inadequate and the full density is required. The decomposition is validated by a self-test that reconstructs an exact brute-force coupling between two compact, well-separated model densities to better than $0.1\%$.

Because protein conformational dynamics modulate the inter-chromophore geometry and hence $J$, we characterised the coupling over a room-temperature trajectory of the actual VenusA206 tandem-dimer construct (see Methods, \textbf{Figure~\ref{fig:tandem}}). The covalently-tethered tandem holds the A206 interface docked for the full $1~\mathrm{ns}$: the chromophore separation fluctuates only over $24$--$26~\mathrm{\AA}$ about its ${\sim}25~\mathrm{\AA}$ mean, and the coupling stays stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$ (splitting $2|J| \approx 221~\mathrm{cm^{-1}}$), consistent with the energy-minimised value. The covalent linker of the tandem construct therefore sustains the strongly-coupled geometry throughout the dynamics, confirming that the coupling is a genuine property of the tethered dimer.

Nguyen et al.\ inferred an apparent coupling strength $U = 131$--$186~\mathrm{cm^{-1}}$ from the CD spectra of a dVenus tandem dimer, corresponding under their $\Delta E = 2U$ convention to a Davydov splitting of $262$--$372~\mathrm{cm^{-1}}$ \cite{nguyen2025anomalous}. Our tandem coupling ($2|J| \approx 221~\mathrm{cm^{-1}}$) lies below this range, though within the same order of magnitude, placing the Venus dimer interaction firmly within the intermediate excitonic coupling regime ($10 - 1000~\mathrm{cm^{-1}}$) \cite{nguyen2025anomalous, kim2019venusa206}.
...
\begin{figure*}[t]
  ...
  \caption{Dynamics of the VenusA206 tandem dimer ($1000$-frame, $1$\,ns NVT; \texttt{build\_tandem\_dimer.py}, \texttt{coupling\_ensemble.py}). (a) Chromophore-centroid separation versus time: the covalent linker holds the A206 interface docked throughout, the separation fluctuating only over $24$--$26~\mathrm{\AA}$ about its ${\sim}25~\mathrm{\AA}$ mean (dashed). (b) Davydov coupling $|J|$ per snapshot, stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$ (dashed line). (c) Distribution of $J$ over the tandem trajectory, tightly clustered around the mean.}
  \label{fig:tandem}
\end{figure*}

\section{Computational Methods}

The quantum-classical hybrid workflow is automated via a single Python orchestrator, \texttt{reproduce\_paper.py}, that integrates structural preparation, classical relaxation, QM/MM electronic-structure calculations (TDDFT, DLPNO-STEOM-CCSD, and the EOM-CCSD(fT)/ADC(2) validation), GPU-accelerated TDC analysis, and the thermal ensemble, spanning the TeraChem, ORCA and Q-Chem back-ends behind a common configuration (random seed, dielectric, ensemble size, per-stage caching). Algorithm \ref{alg:pipeline} serves as a concise, reproducible map of this orchestration---linking each manuscript quantity to the routine that produces it in the released code---rather than introducing new methodology; readers interested only in the physical results may skip it.
...
\begin{enumerate}
    ...
    \item \textbf{TDDFT reference (TeraChem):} As a single-excitation reference and to visualise the transition density, excited-state roots were computed via TDDFT in TeraChem \cite{terachem2009,seritan2021terachem}, treating the monomer's QM region within the point-charge embedding of its classically relaxed MM environment. The QM region comprised $238$ atoms ($222$ atoms treated quantum-mechanically plus $16$ hydrogen link atoms capping the severed backbone bonds): the deprotonated CR2 chromophore together with its first-shell residues (Arg, Asn, Gln, His, Leu, Thr, Tyr, Val) and the five nearest crystallographic/solvating water molecules, with all remaining protein and solvent atoms entering as MM point charges. This stage utilised the range-separated hybrid functional $\omega$B97X-D3/6-311G** \cite{mardirossian2017} with a non-equilibrium COSMO polarisable continuum model \cite{klamt1993cosmo} for the bulk aqueous solvent.

    \item \textbf{Correlated site energy and transition density (ORCA):} Because the anionic chromophore carries appreciable double-excitation character, the site energy and the transition density used for the coupling were computed with DLPNO-STEOM-CCSD \cite{nooijen1997steom,dutta2016dlpno,dutta2018ipeom,dutta2019eaeom,riplinger2013dlpno} as implemented in ORCA 6.1 \cite{neese2020orca,neese2022orca}, using the def2-SVPD basis \cite{weigend2005def2,rappoport2010svpd} with the RIJCOSX approximation, TightSCF/TightPNO thresholds and an automatically constructed auxiliary basis. To keep the correlated calculation tractable while retaining the residue that controls the in-protein shift, the STEOM QM region was reduced to the $44$-atom anionic chromophore plus the $\pi$-stacked Tyr203 (hydrogen-capped, net charge $-1$), embedded in the same MM point-charge field; the active space was fixed deterministically through the DLPNO-STEOM double-excitation filter so that the reported bright state is reproducible run-to-run. The STEOM natural transition orbitals define the transition density $\rho^{g\to e}$, scaled to the STEOM oscillator strength.
...
{Because protein conformational dynamics modulate the inter-chromophore geometry and hence $J$, we characterised the coupling not at a single geometry but over a room-temperature trajectory of the \emph{actual experimental construct}. We built the VenusA206 tandem dimer (VenusA206-TD): two Venus $\beta$-barrels covalently joined by the flexible ${\sim}33$-residue inter-domain linker used in the tandem constructs, with the barrels docked at the physiological A206 interface (residues clustered around Ala206, Leu207, Leu221 and Phe223; released tool \texttt{build\_\allowbreak tandem\_\allowbreak dimer.py}). We ran the tandem in bulk solvent under NVT ($1000$ snapshots, $1$\,ns) and evaluated the transition-density coupling on every frame (\texttt{coupling\_ensemble.py}). The covalent linker holds the A206 interface docked for the entire trajectory: the chromophore-centroid separation stays at $25~\mathrm{\AA}$ (range $24$--$26~\mathrm{\AA}$) with no dissociation, and the coupling remains stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$ throughout, consistent with the energy-minimised value (${\sim}100~\mathrm{cm^{-1}}$). The covalent linker of the tandem construct thus maintains the docked, strongly-coupled geometry throughout the dynamics.}

\section{Summary}

We have characterised the excitonic coupling and energy-transfer dynamics in a Venus dimer using a hybrid QM/MM transition-density approach in an open-quantum-system model. Using a correlated (DLPNO-STEOM-CCSD) site energy and transition density---validated against an explicit triples benchmark---and simulating the actual VenusA206 tandem-dimer construct, we find that the covalent inter-domain linker holds the A206 interface docked throughout a nanosecond of dynamics, keeping the coupling stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$ ($100~\mathrm{cm^{-1}}$ at the energy-minimised geometry), a Davydov splitting $\Delta E \approx 221~\mathrm{cm^{-1}}$ within the intermediate excitonic regime. By integrating distributed transition densities, we show that the standard PDA underestimates the interaction by a factor of $\sim 4.4$, indicating that near-field multipolar effects must be treated explicitly at these biologically relevant distances.

Our analysis validates the role of non-equilibrium dielectric screening in protein environments. Because the excitonic interaction timescale is significantly faster than the macroscopic Debye relaxation time of bulk water ($\sim 8.3$\,ps), the excitonic coupling can operate in the optical dielectric limit. This inertial ``freezing'' of the solvent protects the initial coupling from overwhelming equilibrium screening with $\varepsilon_{\rm eq}$. The stochastic trajectories illustrate that this coherence is transient and environmental interactions drive the system into a localised pointer basis on a sub-$100$\,fs timescale. This temporal separation reconciles the Davydov splitting imprinted at absorption with rapid localisation and subsequent screening within picosecond timescales. The argument is not specific to Venus: because Debye relaxation of aqueous environments universally lies in the picosecond range while optical absorption and dephasing are far faster, the same optical-limit imprinting of the coupling is expected in other water-exposed multichromophore complexes, with the $\beta$-barrel here additionally suppressing the fluctuation amplitude. Although Fig.~\ref{fig:ExcitonDynamics} well explains the single-photon excited subspace phenomena in the Venus dimer, expanding the time-dependent simulation of excitonic interaction to include higher energy levels would offer a more comprehensive view of non-equilibrium dynamics in fluorescent proteins.
...
\Procedure{PipelineMain}{}
    ...
    \State \Call{Stage2\_TDDFT}{}      \Comment{TeraChem: single-excitation reference}
    \State \Call{Stage3\_STEOM}{}      \Comment{ORCA: correlated site energy + density}
    \If{validation requested}          \Comment{Stage 4 is optional (skipped in ORCA-only mode)}
