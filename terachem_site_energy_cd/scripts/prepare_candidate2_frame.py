...
# Candidate-2 register: two chains, each with CR2 at 66 and stacked Tyr at 202
# (verified: CR2 centroid -> Tyr202 CZ = 4.3 A in both chains).
SITE_KEYS = {
    "A": {"cr2": ("A", "66"), "tyr": ("A", "202")},
    "B": {"cr2": ("B", "66"), "tyr": ("B", "202")},
}
...
def load_or_build_charges(
    ...
    *,
    ...
    forcefield_label = (
        "amber19-all.xml + amber14/opc.xml; CR2 RESP from configured AMBER "
        "prmtop by exact atom name"
        if retain_partner_cr2_charges
        else "amber19-all.xml + amber14/opc.xml; CR2 wholly QM"
    )
    if cache.exists():
...
def main() -> None:
        ...
        write_field(
            ...
            policy_description=(
                ...
                if (
                    ...
                    f"{args.radius:g} A AMBER19/OPC electrostatic embedding"
                )
            ),
        )
        shutil.copy2(
            Path(__file__).resolve().parents[1] / "input_templates/tddft_energy_camb3lyp.in",
            site_dir / "tddft.in",
        )
        site_summaries[site] = {
    ...
    summary = {
        ...
        "forcefield": (
            "amber19-all.xml + amber14/opc.xml MM charges; partner CR2 "
            "RESP charges from configured AMBER prmtop"
            if args.retain_partner_cr2_charges
            else (
                "amber19-all.xml + amber14/opc.xml MM charges; "
                "CR2 wholly QM"
            )
        ),
