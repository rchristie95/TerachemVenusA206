...
def stage1_parse_args(argv=None):
    ...
    parser.add_argument(
        "--qm-selection-file",
        default="",
        help="JSON file of frozen QM residue keys. If it exists, the exact same residues are used "
             "(coordinate-independent) for a reproducible QM region across NVT frames; otherwise the "
             "distance-based selection is computed and saved here. Default: <workdir>/qm_selection.json",
    )
...
def residue_key(residue):
    """Coordinate-independent identity of a residue.

    The MD topology (chain id, residue number, residue name) is invariant under
    the stochastic NVT dynamics -- only the coordinates change. Keying the QM
    region on this identity (rather than on a per-frame distance cutoff) lets us
    freeze a single residue selection and reuse it identically across every
    frame/run, which is essential for a reproducible QM region in an ensemble.
    """
    chain = getattr(getattr(residue, "chain", None), "id", "") or ""
    return f"{chain}:{residue.id}:{residue.name}"


def save_qm_selection(selected_residues, path):
    """Persist the QM residue selection as stable keys (for reuse across frames)."""
    keys = sorted(residue_key(r) for r in selected_residues)
    Path(path).write_text(json.dumps({"qm_residue_keys": keys}, indent=2))
    return keys


def load_qm_selection(path):
    data = json.loads(Path(path).read_text())
    return set(data["qm_residue_keys"])
...
def select_qm_residues(
    ...
    # Frozen path: pick EXACTLY the residues named in frozen_keys, independent of
    # the current coordinates. Guarantees an identical QM region across NVT
    # frames despite the stochastic dynamics moving atoms across the cutoff.
    if frozen_keys:
        frozen_keys = set(frozen_keys)
        present = {residue_key(r): r for r in residues}
        missing = frozen_keys - set(present)
        if missing:
            raise RuntimeError(
                "Frozen QM selection references residues absent from this topology: "
                f"{sorted(missing)}. The trajectory topology must match the reference."
            )
        selected = {present[k] for k in frozen_keys}
        selected.update(cr2_residues)  # core is always QM
        n_frozen_water = sum(1 for r in selected if r.name in WATER_RESIDUE_NAMES)
        return (selected, cr2_residues, n_frozen_water, n_frozen_water, None)
...
def stage1_main(argv=None):
    ...
    # Reproducible QM region: if a selection file exists, reuse that exact
    # residue set (coordinate-independent) so every run/frame has an identical
    # QM region despite the stochastic NVT moving atoms across the cutoff.
    # Otherwise select by distance now and persist it for reuse.
    selection_file = Path(args.qm_selection_file) if args.qm_selection_file else (workdir / "qm_selection.json")
    frozen_keys = load_qm_selection(selection_file) if selection_file.exists() else None
    if frozen_keys:
        print(f"    - Using frozen QM residue selection from {selection_file} ({len(frozen_keys)} residues)")
    (
        qm_residues,
        _,
    ...
    if not frozen_keys:
        saved = save_qm_selection(qm_residues, selection_file)
        print(f"    - Saved QM residue selection ({len(saved)} residues) to {selection_file} for reproducible reuse")
...
# absorption_cd_spectra.py) can reuse them without importing OpenMM. The shared
# implementation includes the corrected reciprocal-Angstrom conversion.
...
def stage3_main(argv=None):
    ...
    parser.add_argument("--strict-renorm", action="store_true",
                        help="Abort if the transition-density renormalization factor is far from 1.0 "
                             "(signals a wrong density root / QM-region problem)")
    parser.add_argument(
        "--backend",
    ...
    if dip_mag > 1e-6:
        ...
        # Sanity guard: the grid transition dipole should already match the
        # oscillator-strength target (scale ~ 1). A large factor means the
        # density file is not the selected bright state (e.g. a dark/CT root was
        # plotted) -- the coupling that follows would be meaningless.
        if not (0.5 <= scale <= 2.0):
            msg = (f"renormalization factor {scale:.2f} is far from 1.0: the plotted transition "
                   f"density (grid |mu|={dip_mag:.4f} a.u.) does not match the target bright state "
                   f"(|mu|={mu_target:.4f} a.u.). Likely a wrong density root / QM-region problem.")
            if getattr(args, "strict_renorm", False):
                print(f"[!] ABORT: {msg}")
                sys.exit(3)
            print(f"    - WARNING: {msg}")
        q_opt *= scale
...
def build_stage2_analysis_module():
    ...
    def select_brightest_state(states, nm_min=450, nm_max=600, f_min=0.1):
        if not states:
            return None
        in_window = [state for state in states if nm_min <= state["nm"] <= nm_max]
        if in_window:
            return max(in_window, key=lambda state: state["osc"])["root"]
        # Nothing in the nominal visible window. Distinguish two cases:
        #  (a) a strong bright state exists but blue of the window -- this is the
        #      EXPECTED wB97X-family overestimation of the absolute excitation
        #      energy of (anionic) FP chromophores. The transition-density
        #      coupling computed downstream is energy-independent, so this state
        #      is the correct chromophore band to use; note it, don't alarm.
        #  (b) no bright state at all -- a genuine QM-region/protonation problem.
        bright = [state for state in states if state["osc"] >= f_min]
        if bright:
            best = max(bright, key=lambda state: state["osc"])
            print(f"    - NOTE: brightest state (root {best['root']}, f={best['osc']:.3f}) is at "
                  f"{best['nm']:.0f} nm, blue of the nominal {nm_min}-{nm_max} nm window. For anionic FP "
                  f"chromophores this is the expected wB97X blue bias in the *absolute* energy; the "
                  f"transition-density coupling is energy-independent. Using it as the chromophore band.")
            return best["root"]
        best = max(states, key=lambda state: state["osc"])
        print(f"    - WARNING: no bright state (f>={f_min}) found; brightest is root {best['root']} at "
              f"{best['nm']:.0f} nm (f={best['osc']:.3f}). QM region / pocket protonation likely needs attention.")
        return best["root"]
    ...
    def generate_densities(geom_path, charges_path, root):
        ...
        # Solve at least as many CIS states as the energy run (mod.NUM_STATES) so the
        ...
        cis_states = max(target_root, int(mod.NUM_STATES))
