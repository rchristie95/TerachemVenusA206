...
To compare like with like, we express the $1000$ frame-specific couplings and transition-dipole geometries in the plotting and subtraction convention of Nguyen et al.\ (\textbf{Figure~\ref{fig:spectra}}; \texttt{make\_nguyen\_style\_spectra.py}). The converged STEOM site energy is $19088.2~\mathrm{cm^{-1}}$ ($523.90~\mathrm{nm}$). For each frame, the two Davydov branches lie at $E_0\pm J_i$ and are broadened by a homogeneous Lorentzian half-width $1/(2\pi cT_2^{*})=88.48~\mathrm{cm^{-1}}$ for $T_2^{*}=60$\,fs; explicit averaging over the $J_i$ and dipole geometries incorporates the observed ensemble disorder. Their mean photon wavelengths are $520.69$ and $527.12~\mathrm{nm}$ ($\Delta\lambda=6.43~\mathrm{nm}$). The rotational strengths $R_{\pm}=\mp(\pi E_0/2)\,\mathbf{R}_{LR}\cdot(\boldsymbol{\mu}_L\times\boldsymbol{\mu}_R)$ give the interaction-induced tandem CD couplet.

Nguyen's experimental observable is $\Delta[\theta]=[\theta]_{\rm TDX}-[\theta]_{\rm TD}$ \cite{nguyen2025anomalous}. Our electronic-structure data do not contain the intrinsic monomer/TDX magnetic-dipole spectrum, so Fig.~\ref{fig:spectra}(a) deliberately shows only the interaction-induced contribution: an uncoupled reference obtained by setting $J_i=0$ and the coupled tandem result. In this reduced two-state model the equal and opposite rotational-strength contributions cancel at $J=0$, and the supported Nguyen-style numerical difference is therefore $\Delta\mathrm{CD}_{\rm calc}=\mathrm{CD}_{J=0}-\mathrm{CD}_{J_i}=-\mathrm{CD}_{\rm tandem}$. This subtraction produces the same negative-short-wavelength/positive-long-wavelength sign order as the dVenus difference spectrum, without claiming an absolute molar ellipticity.

Figure~\ref{fig:spectra}(b) fits that numerical difference with the same three-Lorentzian functional form used by Nguyen et al.  The two data-supported components are centred at $520.70$ and $527.11~\mathrm{nm}$, with fitted half-widths of $89.10~\mathrm{cm^{-1}}$. Retaining Nguyen's third-component basis at $481.37~\mathrm{nm}$ gives a fitted amplitude of only $1.5\times10^{-4}$ relative to the unit main lobes (normalized-fit RMS residual $2.1\times10^{-4}$), demonstrating that the present S1-only two-state calculation does not predict that third experimental band. Figure~\ref{fig:spectra}(c) reproduces Nguyen's complete experimental Fig.~4 without modification. Thus the calculated spectrum now uses the experimental subtraction convention, while the remaining width and third-band differences transparently identify physics absent from the present model rather than a discrepancy in the TDC-derived splitting.
...
\begin{figure*}[p]
  ...
  \begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Fig_Spectra_NguyenStyle_Traces.pdf}
    \caption{}
    \label{fig:spectra_traces}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Fig_Spectra_NguyenStyle_Difference.pdf}
    \caption{}
    \label{fig:spectra_difference}
  \end{subfigure}

  \par\medskip
  \begin{subfigure}[b]{0.46\textwidth}
    \centering
    \ifdefined\ForceNguyenPlaceholder
      \fbox{\parbox[c][0.25\textheight][c]{0.90\linewidth}{\centering
        \textbf{Third-party experimental panel not distributed.}\\[4pt]
        See Nguyen et al., \emph{Biophys. J.} \textbf{124}, 4293--4309
        (2025), Fig.~4, DOI: 10.1016/j.bpj.2025.10.022.}}
    \else
      \IfFileExists{Nguyen2025_Figure4_unmodified.jpg}{%
        \includegraphics[width=\linewidth]{Nguyen2025_Figure4_unmodified.jpg}%
      }{%
        \fbox{\parbox[c][0.25\textheight][c]{0.90\linewidth}{\centering
          \textbf{Third-party experimental panel not distributed.}\\[4pt]
          See Nguyen et al., \emph{Biophys. J.} \textbf{124}, 4293--4309
    ...
    \fi
    \caption{}
    \label{fig:spectra_nguyen}
  \end{subfigure}

  \caption{Nguyen-style presentation of the numerical excitonic CD spectrum derived from the $1000$-frame production ensemble, together with the experimental reference \cite{nguyen2025anomalous}. (a) Interaction-induced CD contributions for the uncoupled $J=0$ reference (blue) and coupled tandem ensemble (red). These are the exciton-chirality terms supported by the calculation, not complete absolute TDX and TD molar-ellipticity spectra. (b) Numerical difference in Nguyen's $\mathrm{TDX}-\mathrm{TD}$ subtraction order (black) and the corresponding three-Lorentzian-form fit (red dashed). The two active fitted centres are $520.70$ and $527.11~\mathrm{nm}$; the retained $481.37~\mathrm{nm}$ test component has relative amplitude $1.5\times10^{-4}$ and is absent at the accuracy of the model. (c) Experimental reference: complete Fig.~4 from Nguyen et al.; its internal labels (a)--(f) are the source labels. When the separately retained local asset is available, it is shown unmodified under CC BY-NC-ND 4.0; otherwise the attributed placeholder directs the reader to Nguyen et al., \emph{Biophys. J.} \textbf{124}, 4293--4309 (2025), DOI: 10.1016/j.bpj.2025.10.022. The third-party image is not distributed with this repository.}
  \label{fig:spectra}
\end{figure*}
