...
Homodimers of the yellow fluorescent protein variant dimeric Venus challenge this assumption. Operating at room temperature, these dimers exhibit distinct Davydov splitting in circular dichroism (CD) spectra, and photon antibunching statistics confirm that the interacting chromophores act as a collective quantum entity upon photon absorption \cite{kim2019venusa206, freed2025effect}. These signatures indicate an interaction strength firmly within the intermediate excitonic regime. {Couplings of this magnitude, and substantially larger, are well documented in photosynthetic pigment--protein complexes~\cite{scholes2003, van2000photosynthetic, ishizaki2009unified}; the open question here is therefore not the magnitude of $J$ by itself but the persistence of its coherent spectroscopic signatures at room temperature, in an environment otherwise expected to induce strong decoherence.}
...
\begin{table}[t]
\centering
\caption{Vertical excitation energy of the bright $\pi\pi^{*}$ state of the anionic Venus chromophore. TDDFT yields a single visible bright root (all higher roots dark, $f<0.05$) but blueshifts it; STEOM-CCSD, adopted here as the method of record, agrees with the measured absorption. In-protein values include the point-charge (MM) embedding described in the Methods.}
\label{tab:roots}
...
Experiment (absorption)    & 2.41 & $\sim$515 & n/a \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{itemize}
    \item \textbf{Point-Dipole Approximation:} $J_{\text{PDA}} = 23~\mathrm{cm^{-1}}$ ($2.85~\mathrm{meV}$),
    \item \textbf{Near-field enhancement:} $J_{\text{TDC}}/J_{\text{PDA}} \approx 4.4$.
\end{itemize}}

\noindent For comparison, evaluating the same TDC pipeline on the TDDFT transition density of the identical transition gives $J_{\text{TDC}} = 74~\mathrm{cm^{-1}}$ versus $J_{\text{PDA}} = 13~\mathrm{cm^{-1}}$; the near-field enhancement over the PDA is thus a robust, method-consistent feature, with the more strongly peaked STEOM density coupling ${\sim}1.4$ times more strongly than TDDFT at fixed geometry.
...
Because protein conformational dynamics modulate the inter-chromophore geometry and hence $J$, we characterised the coupling over a room-temperature trajectory of the actual VenusA206 tandem-dimer construct (see Methods, \textbf{Figure~\ref{fig:tandem}}). The covalently-tethered tandem holds the A206 interface docked for the full $1~\mathrm{ns}$: the chromophore separation fluctuates only over $24$--$26~\mathrm{\AA}$ about its ${\sim}25~\mathrm{\AA}$ mean, and the coupling stays stable at $J = 111 \pm 3~\mathrm{cm^{-1}}$ (splitting $2|J| \approx 221~\mathrm{cm^{-1}}$), which we take as the definitive coupling for the Venus dimer. The covalent linker of the tandem construct therefore sustains the strongly-coupled geometry throughout the dynamics, confirming that the coupling is a genuine property of the tethered dimer.
...
To connect the coupling and the open-system parameters to a measurable observable, we convert them into excitonic absorption and circular-dichroism (CD) lineshapes (\textbf{Figure~\ref{fig:spectra}}; tool \texttt{absorption\_cd\_spectra.py}). The two Davydov eigenstates $\ket{\pm}$ are placed at $E_0 \pm J$ and each is broadened as a Voigt profile that combines a homogeneous Lorentzian of half-width $1/(2\pi c\,T_2^{*}) \approx 88~\mathrm{cm^{-1}}$ (at $T_2^{*}=60$\,fs) with an inhomogeneous Gaussian whose width is the $\pm 3~\mathrm{cm^{-1}}$ thermal spread of $J$ over the tandem ensemble (Fig.~\ref{fig:tandem}(c)); because the tethered dimer is rigid, this inhomogeneous width is small and the lineshape is effectively homogeneously ($T_2^{*}$-)limited. That the Davydov splitting survives this dephasing to remain observable is not automatic: the homogeneous linewidth $1/(\pi c\,T_2^{*})$ drops below the splitting $2|J|$ only for $T_2^{*} \gtrsim 48$\,fs, so the exciton doublet is spectrally resolvable across the physically relevant dephasing range, with the adopted $60$\,fs sitting just inside this regime (\textbf{Figure~\ref{fig:spectra}(a)}). The exciton transition dipoles $\boldsymbol{\mu}_{\pm} = (\boldsymbol{\mu}_L \pm \boldsymbol{\mu}_R)/\sqrt{2}$ set the absorption intensities of the Davydov doublet, split by $2|J| \approx 221~\mathrm{cm^{-1}}$ (\textbf{Figure~\ref{fig:spectra}(b)}), while the exciton rotational strengths $R_{\pm} = \mp(\pi E_0/2)\,\mathbf{R}_{LR}\cdot(\boldsymbol{\mu}_L\times\boldsymbol{\mu}_R)$ produce the conservative bisignate CD couplet characteristic of a chiral dimer, its sign fixed by the handedness of the transition-dipole arrangement (\textbf{Figure~\ref{fig:spectra}(c)}). The computed splitting $2|J| \approx 221~\mathrm{cm^{-1}}$ lies just below the window inferred by Nguyen et al.\ from the dVenus CD spectrum ($2U = 262$--$372~\mathrm{cm^{-1}}$) \cite{nguyen2025anomalous}, of the same order and consistent with an intermediate-coupling excitonic dimer, so the open-system parameters are now tied to a spectroscopic signature rather than left purely illustrative.

\begin{figure*}[t]
  \centering
  \begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Fig_Spectra_Resolvability.pdf}
    \caption{}
    \label{fig:spectra_resolv}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Fig_Spectra_Absorption.pdf}
    \caption{}
    \label{fig:spectra_abs}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Fig_Spectra_CD.pdf}
    \caption{}
    \label{fig:spectra_cd}
  \end{subfigure}

  \caption{Excitonic absorption and CD lineshapes derived from the computed coupling and dephasing (\texttt{absorption\_cd\_spectra.py}). (a) Spectroscopic resolvability: the homogeneous linewidth $1/(\pi c\,T_2^{*})$ (blue) falls below the Davydov splitting $2|J| \approx 221~\mathrm{cm^{-1}}$ (dashed) once $T_2^{*} \gtrsim 48$\,fs (shaded region), so the exciton doublet is spectrally resolvable across the physically relevant dephasing range; the adopted $T_2^{*} = 60$\,fs (marker) sits just inside this regime. (b) Excitonic absorption: a Davydov doublet with bands at $E_0 \pm J$ split by $2|J| \approx 221~\mathrm{cm^{-1}}$, each a Voigt profile whose homogeneous width follows from $T_2^{*} = 60$\,fs and whose (small) inhomogeneous width is the thermal spread of $J$ over the tandem ensemble (Fig.~\ref{fig:tandem}(c)). (c) Bisignate CD couplet from the exciton rotational strengths; green bands mark the experimental Davydov splitting inferred by Nguyen et al.\ ($U = 131$--$186~\mathrm{cm^{-1}}$ per band) \cite{nguyen2025anomalous}. The couplet amplitude and sign depend on the three-dimensional arrangement of the two transition dipoles.}
  \label{fig:spectra}
\end{figure*}
