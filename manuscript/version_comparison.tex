...
\texttt{tandem\_dimer\_2.tex}:\\ what changed, and how the CD simulation works}}
\author{}
\date{\vspace{-1.2cm}}
...
\begin{tabular}{lll}
\toprule
& \textbf{Round 1} (\texttt{JPCB\_tandem.tex}) & \textbf{Round 2} (\texttt{tandem\_dimer\_2.tex}) \\
\midrule
Title claim & time-dependent excitonic coupling & site-energy detuning explains the \emph{apparent} coupling \\
...
Detuning $\Delta$ & not computed; degeneracy assumed & $\langle|\Delta|\rangle = 603~\mathrm{cm^{-1}}$ \\
Eigenstates & delocalised exciton & localised, mean mixing $0.18$ \\
Agreement with CD & $J$ within $10\%$ of $130.79$ & $\Omega$, not $2J$, matches; $J$ is $4\times$ smaller \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
CD couplet separation & $14.6 \pm 0.3$~nm (Kim 2019) & $\Omega \Rightarrow 14.75$~nm & \textbf{agrees, no free parameter} \\
...
The decisive point is the first row. Kim's splitting has been read throughout
the literature as $2|J|$, giving $J = 274~\mathrm{cm^{-1}}$; at the
crystallographic $25.4$~\AA{} separation that requires a transition dipole of
$\approx 29$~D or a separation of $\approx 10$~\AA. Read instead as $\Omega$,
the computed $J = 32.82$ and $|\Delta| = 550~\mathrm{cm^{-1}}$ give $14.75$~nm
with nothing adjusted. The result is also robust to the two open systematics
--- the transition dipole ($7.9$ vs $9.6$~D) and whether optical screening is
applied --- which together move $J$ between $22$ and $39~\mathrm{cm^{-1}}$ but
move the predicted splitting only from $14.69$ to $14.79$~nm, precisely because
$\Delta$ dominates $\Omega$.
