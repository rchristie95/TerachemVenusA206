...
Reuses the validated coupling_core pipeline (the same one that gave J_TDDFT=74.38 cm^-1):
align the monomer (in the STEOM density frame) onto dimer chains A/B, transform the density,
Coulomb double-sum. Optical-limit screening eps=1.78 (t=0), as in the paper."""
...
print(f"  -- for reference: J_TDC(TDDFT, published) = 74.38 cm^-1, 2|J|=149, exp 131-186 --")
...
print(f"  near-field enhancement TDC/PDA = {abs(J_cm/(Jdd*HARTREE_CM)):.1f}x")
