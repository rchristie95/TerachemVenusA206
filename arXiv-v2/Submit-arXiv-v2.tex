...
$J = 74.38~\mathrm{cm^{-1}}$ against a point-dipole estimate of
$13.31~\mathrm{cm^{-1}}$ at a separation of $27.6~\mathrm{\AA}$, and attributed
the resulting factor of $5.6$ to near-field multipolar effects. That ratio was
an artefact of a unit conversion in the Coulomb sum, compounded by comparing two
numbers computed on different geometries with different transition densities.
Corrected on matched frames, density and geometry, the multipolar correction is
$1.19\times$, not $5.6\times$: the point-dipole approximation is adequate at
this separation, reversing the conclusion of v1. Version 1 also argued that the
resulting splitting agreed with a coupling inferred from a circular-dichroism
couplet separation; that inference is withdrawn, for reasons set out below.
Readers who have cited the $5.6\times$ near-field result should use this version
instead.}

We quantify excitonic coupling in a Venus fluorescent-protein homodimer using correlated electronic structure, transition-density integration, molecular dynamics, and an open-system model. Domain-based local-pair-natural-orbital similarity-transformed equation-of-motion coupled-cluster theory (DLPNO-STEOM-CCSD) places the embedded bright $\pi\pi^{*}$ state at $523.90~\mathrm{nm}$, near the $515~\mathrm{nm}$ absorption maximum, whereas time-dependent density functional theory within the Tamm--Dancoff approximation yields $360.9~\mathrm{nm}$. Across $1000$ snapshots of the VenusA206 tandem dimer, the transition-density coupling is $J = 32.8 \pm 1.6~\mathrm{cm^{-1}}$ at $24.69 \pm 0.32~\text{\AA}$, only $1.19$ times the point-dipole estimate of $27.6 \pm 1.3~\mathrm{cm^{-1}}$. A separate gas-phase ORCA control gives $6.09/5.36~\mathrm{meV}$ for the corresponding distributed-density/point-dipole couplings and supports the corrected implementation. The predicted splitting ($65.6~\mathrm{cm^{-1}}$) is about fourfold below the constrained circular-dichroism fit, but that comparison is not well posed. Simulating the couplet with the published lineshape shows its peak-to-peak separation to be a non-monotonic, two-valued and linewidth-dominated function of the exciton gap: a gap of zero still yields extrema $690~\mathrm{cm^{-1}}$ apart, so no coupling can be read from the separation in either direction. QM/MM site energies further show the chromophores are not isoenergetic, with $\langle|\Delta|\rangle \approx 576~\mathrm{cm^{-1}}$, an order of magnitude above $2|J|$; this localises the eigenstates and suppresses the couplet amplitude by $2|J|/\Omega=0.12$, the amplitude rather than the separation being the observable that carries $J$. An independent two-photon polarisation-ratio measurement places the coupling at $32$--$40~\mathrm{cm^{-1}}$ without dielectric screening, against $58~\mathrm{cm^{-1}}$ for the present calculation on the same convention; the residual factor of $1.6$ is dominated by the transition-dipole magnitude, $9.8~\mathrm{D}$ from the spectroscopically normalised density here against $7.6$--$7.9~\mathrm{D}$ implied by the experimental analyses. Vertical excitation justifies optical-limit dielectric screening, but for the illustrative pure-dephasing time of $60$\,fs the estimated homogeneous linewidth ($177~\mathrm{cm^{-1}}$) exceeds the splitting. A diffusive stochastic-wavefunction model therefore illustrates post-absorption dephasing rather than establishing a resolved doublet.
\end{abstract}
...
\newpage
Nguyen et al.\ analysed the $\mathrm{dVenus\mbox{-}TDX}-\mathrm{dVenus\mbox{-}TD}$ difference-molar-ellipticity spectrum with three Lorentzians \cite{nguyen2025anomalous}. Their constrained sensitivity fit (Table~S3) places the excitonic pair at $514.07$ and $521.07~\mathrm{nm}$, giving $U=130.79~\mathrm{cm^{-1}}$ and $2U=261.58~\mathrm{cm^{-1}}$, with a third band at $481.37~\mathrm{nm}$; the unconstrained fit shown in their main Fig.~4(d) instead gives $511.8$ and $521.7~\mathrm{nm}$, or $U\approx185.4~\mathrm{cm^{-1}}$. These are alternative fit conventions rather than the endpoints of a statistical confidence interval. Our corrected ensemble value, $J=32.8~\mathrm{cm^{-1}}$ ($2|J|=65.6~\mathrm{cm^{-1}}$), is about a factor of four below the constrained $U$ value. That factor of four is not a failure of the coupling calculation. Two separate assumptions in the spectroscopic analysis contribute to it. The first is degeneracy: both the constrained fit and the earlier Davydov reading place the two components symmetrically about the monomer energy, which is the degenerate-dimer prediction, and the detuning never enters. The second, which proves to be the larger effect, is that the couplet separation is treated as a Hamiltonian gap at all. QM/MM site energies on the same construct show that the two chromophores are not isoenergetic, with $\langle|\Delta|\rangle$ of several hundred $\mathrm{cm^{-1}}$, an order of magnitude above $2|J|=65.6~\mathrm{cm^{-1}}$. For a detuned pair the exciton gap is $\Omega=\sqrt{\Delta^{2}+4J^{2}}$, and the rotational strength of each band carries a factor $\sin2\theta=2|J|/\Omega$. It is tempting to close the gap between calculation and experiment by identifying the measured couplet separation with $\Omega$, but that step is not valid: $\Omega$ is a Hamiltonian gap, whereas the measurement is the peak-to-peak separation of a bisignate lineshape, and for overlapping bands the two are not equal. Simulating the couplet directly with the lineshape of the constrained fit (half-widths $293.5$ and $439.3~\mathrm{cm^{-1}}$ about $19322~\mathrm{cm^{-1}}$, fitted peak-amplitude ratio $1.58$) makes the size of the effect explicit. The published latent gap of $261.58~\mathrm{cm^{-1}}$ produces extrema separated by $500~\mathrm{cm^{-1}}$, an inflation of $1.93$, and a gap of \emph{zero} still produces $690~\mathrm{cm^{-1}}$. The separation is moreover not monotonic in the gap: it falls from that zero-gap derivative limit to a minimum of $491~\mathrm{cm^{-1}}$ near $\Omega\approx190~\mathrm{cm^{-1}}$ before rising again, so a given separation is consistent with two different gaps, and $\Omega=553.9~\mathrm{cm^{-1}}$ would in fact yield $652~\mathrm{cm^{-1}}$ rather than the observed $548~\mathrm{cm^{-1}}$. The couplet separation is therefore not an estimator of the exciton gap in either direction, and the apparent factor of four between $2|J|$ and the fitted $U$ is an artefact of reading a linewidth-dominated separation as a Hamiltonian gap, not a physical discrepancy to be reconciled. What the detuning does control is the couplet \emph{amplitude}, suppressed by $2|J|/\Omega=0.12$ relative to a degenerate pair, together with the localisation of the eigenstates; the amplitude and its signed first moment are the observables that carry $J$. Reading the same splitting as $2|J|$ instead requires $J=274~\mathrm{cm^{-1}}$, which at the crystallographic $25.4~\text{\AA}$ separation would demand a transition dipole near $29~\mathrm{D}$ or a separation near $10~\text{\AA}$, neither of which is available to two $\beta$-barrels of diameter $\geq22~\text{\AA}$. The same artefact appears, and is correctly handled, in the two-photon work of Cusick et al.: their $\Omega$ spectrum shows extrema separated by $750~\mathrm{cm^{-1}}$ against an H--J splitting of only $60$--$90~\mathrm{cm^{-1}}$, which they identify as the first derivative of a Gaussian with extrema at $\mathrm{FWHM}/\sqrt{2\ln 2}$ and analyse by fitting amplitudes rather than separations \cite{cusick2026twophoton}.

That same study provides the one genuinely independent determination of the coupling, and the comparison requires care over the screening convention. Their spectroscopic value of $32$--$40~\mathrm{cm^{-1}}$ is obtained with no dielectric attenuation, which they omit explicitly on the grounds of the nanometre scale of the system, whereas the value reported here is screened by $\varepsilon_{\rm opt}=1.77$. Like for like, the present calculation gives $58~\mathrm{cm^{-1}}$ unscreened against their $32$--$40~\mathrm{cm^{-1}}$, or $32.8~\mathrm{cm^{-1}}$ against $18$--$23~\mathrm{cm^{-1}}$ screened: the ratio is $1.6$ in either convention, because the screening factor cancels from it. Both determinations therefore place the coupling in the tens of $\mathrm{cm^{-1}}$, two orders of magnitude below the values once inferred from couplet separations, but they are not numerically coincident. Inverting their point-dipole expression for the transition dipole implied by their own $(\bar\upsilon, R, \delta)$ returns $7.6$--$7.9~\mathrm{D}$, against the $9.8~\mathrm{D}$ of the spectroscopically normalised STEOM density used here; since $J\propto|\mu|^{2}$ this single input accounts for a factor of $1.5$ and is the dominant identified systematic in the present calculation.
